...
/*   Updated: 2024/09/30 12:19:05 by yadereve         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		float angle)
// {
// 	pos->x = player.pos.x + distance * cos(angle);
// 	pos->y = player.pos.y + distance * sin(angle);
// }
...
void	vision_point(int fov, int screen_x, float *distance, t_player player)
{
	...
	ray_angl = player.direction + ((fov * (float)(screen_x - data->win_width
					/ 2) / data->win_width));
	// ray_angl = player.direction + ((fov / 2) * (screen_x - (data->win_width / 2)) / data->win_width);
	...
	*distance *= cos(degrees_to_radians(ray_angl - player.direction));
	// *distance *= cos(ray_angl - player.direction);
}
